"""
Run proper PoCo baseline (training-loss local loss) across 15 seeds.
...
RESULTS_BASE = os.path.join(BASE, 'results', 'poco_proper')
...
for seed in SEEDS:
    ...
    csv_path = os.path.join(rdir, f'poco8_seed{seed}_rounds.csv')
    if os.path.exists(csv_path):
        print(f'[{done}/{total}]  poco8_proper  seed={seed}  (cached)')
        continue
    print(f'\n{"="*60}')
    print(f' [{done}/{total}]  poco8_proper  seed={seed}')
    print(f'{"="*60}')
...
poco_f1, poco_rds, poco_fails = load_results(RESULTS_BASE, SEEDS, 'poco8')
...
converged_poco = poco_rds[poco_rds < MAX_ROUNDS]
converged_ao   = ao_rds[ao_rds < MAX_ROUNDS]

print(f"\nPoCo (proper): F1={poco_f1.mean():.2f}±{poco_f1.std():.2f}%  "
      f"Rds={converged_poco.mean():.1f if len(converged_poco) else 0:.1f}  "
      f"Fails={poco_fails}/15")
print(f"AO (ours)    : F1={ao_f1.mean():.2f}±{ao_f1.std():.2f}%  "
      f"Rds={converged_ao.mean():.1f}  Fails={15-len(ao_f1_list)}/15")

min_len = min(len(poco_rds), len(ao_rds))
if min_len >= 5:
    try:
        stat, p = wilcoxon(ao_rds[:min_len], poco_rds[:min_len], alternative='less')
        r = 1 - 2 * stat / (min_len * (min_len + 1))
        print(f"\nWilcoxon (AO < PoCo_proper, rounds-to-82%): stat={stat:.1f}  p={p:.4f}  r={r:.3f}")
