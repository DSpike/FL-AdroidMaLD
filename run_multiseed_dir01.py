"""
5-seed experiment: FedAvg-all vs Random-8 vs AOA-8 at Dir(0.1).
Runs 15 experiments total (3 methods × 5 seeds) sequentially via subprocess.

Results saved to:
    results/dir01_5seed/{method}/seed{N}/{experiment}_rounds.csv
"""
...
SCRIPT  = os.path.join(BASE, 'run_experiment_dir01.py')
...
for method in METHODS:
    for seed in SEEDS:
        ...
        if result.returncode != 0:
            print(f"  *** FAILED: {method} seed={seed} (exit {result.returncode}) ***")
            failed.append((method, seed))

# ── Summary ───────────────────────────────────────────────────────────────────
print(f"\n{'='*60}")
print(" RESULTS SUMMARY  —  Dir(0.1), 5 seeds")
print(f"{'='*60}")
...
for method in METHODS:
    ...
    for seed in SEEDS:
        csv_path = os.path.join(
            BASE, 'results', 'dir01_5seed', method, f'seed{seed}',
