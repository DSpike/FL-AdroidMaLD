...
\item Statistically significant convergence improvements over Oort and PoCo across 15 seeds (paired Wilcoxon, $p<0.05$); AO's zero-failure reliability is sustained across $k\in\{4,\dots,12\}$, with significant improvement over Oort at $k=12$ ($p=0.031$).
\end{itemize}
\vspace{-4pt}
...
\toprule
Method & \makecell{Scoring\\Level} & \makecell{SI\\Optimizer} & Domain & \makecell{Real\\Data} & \makecell{Statistical\\Testing} \\
\midrule
Oort \cite{lai2021oort}, PoCo \cite{cho2022poco} & Per-client & \texttimes & Vision/NLP    & \checkmark & \texttimes \\
DivFL \cite{balakrishnan2022divfl}               & Subset     & \texttimes & Vision/NLP    & \checkmark & \texttimes \\
...
FedDiverse \cite{nemeth2025feddiverse}           & Subset     & \texttimes & Vision        & \checkmark & \texttimes \\
\textbf{Ours}                                    & \textbf{Subset} & \checkmark & \textbf{Malware} & \checkmark & \checkmark \\
\bottomrule
\end{tabular}
\end{table}
...
\end{align*}
where $C=12$ is the number of classes, $\mathcal{C}_i$ is the set of classes present in client $i$'s local data, $p_{i,c}$ is client $i$'s fraction of samples from class $c$, and $\mathbf{p}_i \in \mathbb{R}^C$ is client $i$'s class-proportion vector. $\text{div}(S)$ rewards subsets that collectively diverge from the global model, ensuring informative updates; $\text{cov}(S)$ rewards subsets that collectively cover all $C$ malware classes; $\text{div}_{pair}(S)$ rewards subsets whose members have dissimilar local class distributions, discouraging redundant clients. Prior to computing $F(S)$, $\text{div}(S)$ and $\text{cov}(S)$ are independently min-max normalized to $[0,1]$ across all $n$ clients. We use $\alpha=0.3$, $\beta=0.3$, and $\lambda=0.2$ (note: $\text{div}_{pair}(S)\in[0,2]$ while the other terms are in $[0,1]$, so the effective contribution of $\lambda$ can reach $0.4$); these values were fixed prior to the comparative evaluation reported in Section~V. Section~V-G presents an ablation study suggesting that the divergence term is the primary contributor to performance. The cov$(S)$ and div$_{pair}(S)$ terms use per-client class-proportion vectors $\mathbf{p}_i$ transmitted alongside model updates.
...
C. Dwork, F. McSherry, K. Nissim, and A. Smith, ``Calibrating noise to sensitivity in private data analysis,'' in \emph{Proc. 3rd Theory Cryptography Conf. (TCC)}, New York, NY, USA, 2006, pp. 265--284.
